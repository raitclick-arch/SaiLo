...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.searchFriendAction = async function() {
        ...
        try {
            ...
            container.innerHTML = `<span style="font-size:0.75rem; color:var(--text-muted);">Search failed</span>`;
        }
    };
    ...
    window.sendFriendRequestDB = async function(targetUser) {
        ...
        try {
            ...
            window.showToast("Could not send request");
        }
    };
    ...
    async function fetchFriendsAndRequests() {
        ...
        try {
            ...
            console.warn("Friends sync notice:", err);
        }
    }
    ...
    // Safely sync profile and join realtime room
    syncMyProfile().catch(() => {});
    setupRealtimeRoom();
});
